...
def handle_message(event):
    try:
        ...
        #更新衛生棉庫存回傳資料開頭
            ...
            url_list = ['https://images.pexels.com/photos/7775232/pexels-photo-7775232.png?auto=compress&cs=tinysrgb&dpr=2&h=650&w=940', 'https://images.pexels.com/photos/7775231/pexels-photo-7775231.png?auto=compress&cs=tinysrgb&dpr=1&w=500', 'https://images.pexels.com/photos/7775230/pexels-photo-7775230.png?auto=compress&cs=tinysrgb&dpr=1&w=500']
...
def sendBack(event, backdata, user_id):
    try:
        ...
        #計算本次週期
        this_cycle = datetime.date(int(m_dt[0]), int(m_dt[1]), int(m_dt[2])) - datetime.date(int(last_date[0]), int(last_date[1]), int(last_date[2]))            
        ...
        #將資料寫回資料庫
        sql = "INSERT INTO cycle (userid, pdate, cycle) VALUES('%s', '%s', '%s')"%(user_id, str(dt), str(m_this_cycle))
        db.engine.execute(sql)
...
def update_cotton(event, mtext, user_id):
    try:
        ...
        sql = "UPDATE cotton SET pad = '%s', ldailyuse='%s', ndailyuse='%s', hdailyuse='%s', nnightuse='%s', hnightuse='%s'  WHERE userid = '%s'"%(s_calculate_cotton_list[0], s_calculate_cotton_list[1], s_calculate_cotton_list[2], s_calculate_cotton_list[3], s_calculate_cotton_list[4], s_calculate_cotton_list[5], user_id)
        db.engine.execute(sql)
...
def first_time_set(event, mtext, user_id):
    try:
        ...
        predict_date = datetime.date(int(m_this_date[0]), int(m_this_date[1]), int(m_this_date[2])) + datetime.timedelta(days = int(flist[2]))
        
        #將週期資料寫進週期資料表
        insert_cycle_sql = "INSERT INTO cycle (userid, pdate, cycle) VALUES('%s', '%s', '%s')" %(str(user_id), str(flist[1]), str(flist[2]))
        db.engine.execute(insert_cycle_sql)
        
        #將預測日寫進預測日資料表
        insert_predict_sql = "INSERT INTO predictdate (userid, predictdate) VALUES('%s', '%s')" %(str(user_id), str(predict_date))
        db.engine.execute(insert_predict_sql)
        
        #將庫存寫進庫存資料表
        insert_cotton_sql = "INSERT INTO cotton (userid, pad, ldailyuse, ndailyuse, hdailyuse, nnightuse, hnightuse) VALUES('%s', '%s', '%s', '%s', '%s', '%s', '%s')" %(str(user_id), str(flist[3]), str(flist[4]), str(flist[5]), str(flist[6]), str(flist[7]), str(flist[8]))
...
def find_store(event, latitude, longitude, mtext):
        #建立 list 來存取萃取出的店家資料
        serach_url = "https://maps.googleapis.com/maps/api/place/nearbysearch/json?key={}&location={},{}&rankby=distance&keyword={}&language=zh-TW" .format(key, latitude, longitude, mtext)
        ...
        for i in range(3):
            ...
            map_url_list.append("https://www.google.com/maps/search/?api=1&query={},{}&query_place_id={}".format(latitude_result_list[i], longitude_result_list[i], place_id_result_list[i]))
           
            #製造用於查詢 google map 當中店家截圖的語法
            #切記不能塞NONE進去網址連結，若沒有預設圖片，須自己製作OR隨意指定
            if 'photos' not in json_result['results'][i]:
                photo_url_list.append('https://images.pexels.com/photos/7774217/pexels-photo-7774217.png?auto=compress&cs=tinysrgb&dpr=2&h=650&w=940')
            else:
                photo_ref = json_result['results'][i]['photos'][0]['photo_reference'] #圖片參考 ID
                photo_width = json_result['results'][i]['photos'][0]['width'] #圖片寬度
                #語法來源參考: https://www.tpisoftware.com/tpu/articleDetails/1136
                photo_url_list.append('https://maps.googleapis.com/maps/api/place/photo?key={}&photoreference={}&maxwidth={}'.format(key, photo_ref, photo_width))        
               
            distance = math.sqrt(((latitude - latitude_result_list[i])**2)+((longitude-longitude_result_list[i])**2))
